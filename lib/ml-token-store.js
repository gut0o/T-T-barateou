...
// TT_REFRESH_LOCK_FIX_V1
const REFRESH_LOCK_STALE_MS =
  Math.max(
    Number(process.env.ML_REFRESH_LOCK_STALE_MS || 120000) || 120000,
    30000
  );
...
async function clearStaleRefreshLock(refreshToken) {
  const hash = refreshTokenHash(refreshToken);
  const cutoff =
    new Date(Date.now() - REFRESH_LOCK_STALE_MS)
      .toISOString();

  await supabaseRequest(
    "tt_ml_refresh_locks" +
      `?refresh_hash=eq.${encodeURIComponent(hash)}` +
...
async function releaseRefreshTokenLock(refreshToken) {
  const hash = refreshTokenHash(refreshToken);
...
async function claimRefreshToken(
  ...
  await clearStaleRefreshLock(refreshToken);
...
async function exchangeRefreshToken(
  ...
  let claimed =
    await claimRefreshToken(record.refresh_token);
  ...
  if (!claimed) {
      ...
      const latestIsNewer =
        Boolean(
          ...
          (
            ...
            Number(latest.obtained_at || 0) >
              Number(record.obtained_at || 0)
          )
        );
    ...
    if (!claimed) {
      throw new Error(
        "A renovação do token do Mercado Livre já está em andamento. " +
  ...
  try {
    ...
    try {
      await releaseRefreshTokenLock(
        record.refresh_token
      );
    } catch (releaseError) {
      console.error(
        "⚠️ Não consegui liberar a trava de refresh do Mercado Livre:",
